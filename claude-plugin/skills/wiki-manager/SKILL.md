...
### Stack Pages
See [references/wiki-structure.md](references/wiki-structure.md) § Stack Pages and [references/compilation.md](references/compilation.md).
Flow: scenario/topic/export (e.g. a Perplexity stack export, a scenario like "realtime IoT analytics dashboard", or an existing `raw/` set) → frame the problem and role groups → gather components from `raw/` first, then targeted web research to fill gaps → normalize one canonical entry per (role group, tool) → compare and rate with grounded justifications → write a `category: stack` article to `wiki/stacks/<slug>.md` with the extended frontmatter (`topic`, `scenario`, `llm_usage`, `stack_type`, `related_topics`) and the canonical body (Problem Context, Problem-Oriented Notes, Normalized Stack Table, Comparison & Ratings Table, Stack Overview, LLM Hints) → ingest research-worthy sources into `raw/` and cite them → update stack/wiki/master indexes. Use `/wiki:ingest-stack` for stack pages; it is compiled synthesis, not a raw dump. `--from-sources` assembles only from existing `raw/`.
